...
export default {
            ...
            return h(
              "Dropdown",
              {
                ...
                on: {
                  "on-click": name => {
                    if (name === "details") {
                      this.detailData(params.row.id, params.row.name);
                    } else if (name === "modify") {
  ...
  methods: {
    ...
    formOk() {
      ...
      let obj = { name, type, coordinate_direction, period };
      if (type === "0") {
        obj.mon_day_start = time0[0];
        obj.mon_day_end = time0[1];
      } else if (type === "1") {
        obj.mon_day_start = `${time1[0].getFullYear()}/${time1[0].getMonth() +
          1}/${time1[0].getDate()} ${time1[0].getHours()}:${time1[0].getMinutes()}`;
        obj.mon_day_end = `${time1[1].getFullYear()}/${time1[1].getMonth() +
